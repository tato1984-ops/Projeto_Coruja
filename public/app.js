...
async function init(){
 try{
  ...
  if(s){let o=JSON.parse(s); teams=o.teams||[]; sorteioFeito=!!o.sorteioFeito}
  listenPlayers();
...
function saveSorteio(){localStorage.setItem('coruja_sorteio',JSON.stringify({teams,sorteioFeito})); renderAll()}
...
function drawTeams(){
 const presentes=getPresentPlayers();
 ...
 const candidate=buildBalancedTeams(presentes,sizes);
 const validation=validateDraw(candidate,presentes,sizes);

 if(!validation.ok){
  console.error('Coruja Engine 1.1 - Sorteio inválido:', validation.erro, {candidate,presentes,sizes});
  alert(`Não foi possível gerar um sorteio válido: ${validation.erro}`);
  return;
 }

 teams=candidate;
 sorteioFeito=true;
 console.info('🦉 Coruja Engine 1.1 Build 001', {
  presentes: presentes.length,
  estrutura: sizes.join('/'),
  somas: teams.map(sum),
  medias: teams.map(t=>Number(avg(t).toFixed(2))),
  equilibrio: balancePct(teams)
 });
 saveSorteio();
}

function getPresentPlayers(){
 return players.filter(p=>p.presente && (p.status||'ativo')==='ativo');
}
...
function buildBalancedTeams(presentes,sizes){
 const ordered=orderPlayersForDraw(presentes);
 const teams=sizes.map(()=>[]);

 // Regra oficial: completar os times de 6 antes de abrir o próximo.
 // Ex.: 13 => 6/6/1, 19 => 6/6/6/1, 25 => 6/6/6/6/1.
 const fullTeamCount=sizes.filter(size=>size===6).length;
 const fullPlayerCount=fullTeamCount*6;
 const fullPlayers=ordered.slice(0,fullPlayerCount);
 const extraPlayers=ordered.slice(fullPlayerCount);

 distributeFullTeams(fullPlayers,teams,sizes,fullTeamCount);
 distributeExtraPlayers(extraPlayers,teams,sizes);

 return improveBalance(teams,sizes);
}

function orderPlayersForDraw(presentes){
 // Embaralha antes de ordenar para desempatar jogadores do mesmo nível.
 // Isso mantém o sorteio variado, mas sem quebrar as regras de formação.
 return shuffle([...presentes]).sort((a,b)=>{
  const nivelDiff=Number(b.nivel||5)-Number(a.nivel||5);
  if(nivelDiff!==0) return nivelDiff;
  return String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR');
 });
}

function distributeFullTeams(fullPlayers,teams,sizes,fullTeamCount){
 if(fullTeamCount<=0) return;
 const normal=[...Array(fullTeamCount).keys()];
 const reverse=[...normal].reverse();

 fullPlayers.forEach((player,idx)=>{
  const round=Math.floor(idx/fullTeamCount);
  const order=round%2===0 ? normal : reverse;
  const available=order.filter(teamIndex=>teams[teamIndex].length<sizes[teamIndex]);

  // Entre os times disponíveis da rodada, escolhe o que está com menor soma.
  const chosen=available.sort((a,b)=>{
   const sumDiff=sum(teams[a])-sum(teams[b]);
...
function distributeExtraPlayers(extraPlayers,teams,sizes){
 // Sobras entram exclusivamente no próximo time incompleto.
 // Nunca redistribui para transformar 6/6/6/1 em 5/5/5/4.
 extraPlayers.forEach(player=>{
  const target=teams.findIndex((team,i)=>team.length<sizes[i]);
  if(target>=0) teams[target].push(player);
 });
}

function improveBalance(teams,sizes){
 // Ajuste fino somente entre times completos.
 // Times incompletos representam "próximos" e não devem quebrar a formação oficial.
 let current=teams.map(team=>[...team]);
 const fullIdx=current.map((team,i)=>team.length===6 ? i : -1).filter(i=>i>=0);
 if(fullIdx.length<2) return current;
 ...
 while(improved && guard<100){
  ...
  const currentScore=scoreTeams(current,sizes);
  let bestSwap=null;

  for(const a of fullIdx){
   for(const b of fullIdx){
    if(a>=b) continue;
    for(let ia=0; ia<current[a].length; ia++){
     for(let ib=0; ib<current[b].length; ib++){
      const clone=current.map(team=>[...team]);
      const tmp=clone[a][ia];
      clone[a][ia]=clone[b][ib];
      clone[b][ib]=tmp;

      const validation=validateDraw(clone,getPlayersFromTeams(current),sizes);
      if(!validation.ok) continue;

      const newScore=scoreTeams(clone,sizes);
      if(newScore+0.001<currentScore && (!bestSwap || newScore<bestSwap.score)){
       bestSwap={teams:clone,score:newScore};
      }
     }
    }
   }
...
function getPlayersFromTeams(teams){
 return teams.flat();
}

function validateDraw(teams,presentes,sizes){
 if(!Array.isArray(teams)) return {ok:false,erro:'Estrutura de times inválida'};
 if(!Array.isArray(sizes)) return {ok:false,erro:'Estrutura esperada inválida'};
 if(teams.length!==sizes.length) return {ok:false,erro:`Quantidade de times inválida. Esperado ${sizes.length}, gerado ${teams.length}`};

 for(let i=0;i<sizes.length;i++){
  if(!Array.isArray(teams[i])) return {ok:false,erro:`Time ${i+1} inválido`};
  if(teams[i].length!==sizes[i]) return {ok:false,erro:`Time ${i+1} deveria ter ${sizes[i]} jogadores e ficou com ${teams[i].length}`};
  if(teams[i].length>6) return {ok:false,erro:`Time ${i+1} ficou com mais de 6 jogadores`};
 }

 const ids=[];
 teams.forEach(team=>team.forEach(player=>ids.push(getPlayerKey(player))));
 const unique=new Set(ids);
 if(ids.length!==presentes.length) return {ok:false,erro:`Nem todos os presentes foram distribuídos. Presentes: ${presentes.length}, distribuídos: ${ids.length}`};
 if(unique.size!==ids.length) return {ok:false,erro:'Jogador repetido no sorteio'};

 const presentIds=new Set(presentes.map(getPlayerKey));
 for(const id of ids){
  if(!presentIds.has(id)) return {ok:false,erro:'Jogador sorteado não está na lista de presentes'};
 }
...
function getPlayerKey(player){
 return String(player?.id || player?.jogadorId || player?.nome || Math.random());
}
...
function sum(team){return team.reduce((a,p)=>a+Number(p.nivel||0),0)}
function avg(team){return team.length?sum(team)/team.length:0}
function scoreTeams(teams,sizes){
 const fullTeams=teams.filter(team=>team.length===6);
 const base=fullTeams.length?fullTeams:teams.filter(team=>team.length);
 if(!base.length) return 9999;

 const sums=base.map(sum);
 const avgs=base.map(avg);
 const meanAvg=avgs.reduce((a,b)=>a+b,0)/avgs.length;
 const avgDiff=avgs.reduce((total,value)=>total+Math.abs(value-meanAvg),0);
 const sumDiff=sums.length?Math.max(...sums)-Math.min(...sums):0;
 const sizePenalty=teams.reduce((acc,team,i)=>acc+Math.abs(team.length-(sizes?sizes[i]:team.length))*100,0);

 return avgDiff*10+sumDiff+sizePenalty;
}
...
function balancePct(teams){
 ...
 return Math.max(70,Math.min(100,Math.round((100-diff*8)*10)/10));
}
