...
const CORUJA_ENGINE_VERSION='1.1.2-build002-dashboard';
const CORUJA_BACKUP_KEY='coruja_players_backup_v1';
...
function validatePlayerData(data,currentId=null){
 const nome=cleanText(data.nome);
 const telefone=cleanText(data.telefone);
 const nivel=Number(data.nivel);

 if(nome.length<2) return {ok:false,message:'Informe um nome válido com pelo menos 2 caracteres.'};
 if(!Number.isFinite(nivel) || nivel<1 || nivel>10) return {ok:false,message:'O nível técnico deve estar entre 1 e 10.'};

 const digits=phoneDigits(telefone);
 if(telefone && ![10,11].includes(digits.length)){
  return {ok:false,message:'O telefone deve ter 10 ou 11 números, incluindo o DDD.'};
 }

 const duplicateName=players.find(p=>String(p.id)!==String(currentId) && normalizedName(p.nome)===normalizedName(nome));
 if(duplicateName) return {ok:false,message:`Já existe um jogador cadastrado com o nome “${duplicateName.nome}”.`};

 if(digits){
  const duplicatePhone=players.find(p=>String(p.id)!==String(currentId) && phoneDigits(p.telefone)===digits);
  if(duplicatePhone) return {ok:false,message:`Este telefone já pertence ao jogador “${duplicatePhone.nome}”.`};
 }

 return {ok:true,data:{...data,nome,telefone,nivel}};
}
function backupSafePlayers(){
...
function createPlayersBackup(reason='alteração'){
 try{
  const payload={
   createdAt:new Date().toISOString(),
   reason,
   engineVersion:CORUJA_ENGINE_VERSION,
   players:backupSafePlayers()
  };
  localStorage.setItem(CORUJA_BACKUP_KEY,JSON.stringify(payload));
  return true;
 }catch(e){
  console.error('Falha ao criar backup local:',e);
  return false;
 }
}
function restoreLastPlayersBackup(){
 let payload;
 try{ payload=JSON.parse(localStorage.getItem(CORUJA_BACKUP_KEY)||'null'); }
 catch(e){ payload=null; }

 if(!payload?.players?.length){
  alert('Nenhum backup local de jogadores foi encontrado neste aparelho.');
  return;
 }
 const when=new Date(payload.createdAt).toLocaleString('pt-BR');
 if(!confirm(`Restaurar o backup criado em ${when}?\n\nA restauração atualizará os jogadores atuais.`)) return;

 if(firestoreOk){
  alert('Por segurança, a restauração automática no Firestore ainda não foi liberada. O backup está preservado neste aparelho.');
  return;
 }
...
async function removePlayer(id){
 ...
 if(!player) return;
 if(!confirm(`Excluir o jogador “${player.nome}”?\n\nUm backup local será criado antes da exclusão.`)) return;
 createPlayersBackup(`antes de excluir ${player.nome}`);
 try{
...
function setup(){
 ...
 if(reset && !document.getElementById('restoreBackupBtn')){
  const restore=document.createElement('button');
  restore.id='restoreBackupBtn';
  restore.type='button';
  restore.textContent='Restaurar último backup';
  restore.onclick=restoreLastPlayersBackup;
  reset.insertAdjacentElement('afterend',restore);
 }
}
async function addPlayerFromForm(){
 const draft={
  nome:$('playerName').value,
  tipo:$('playerType').value,
  nivel:Number($('playerLevel').value||5),
  telefone:$('playerPhone').value,
  status:'ativo', whatsapp:'sem_resposta', presente:false, atraso:false, pagamento:'pendente'
 };
 const validation=validatePlayerData(draft);
 if(!validation.ok){alert(validation.message); return;}

 const jogadorId=players.reduce((m,p)=>Math.max(m,Number(p.jogadorId||0)),0)+1;
 const p={...validation.data,jogadorId,createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
 createPlayersBackup(`antes de cadastrar ${p.nome}`);

 try{
  if(firestoreOk) await addDoc(jogadoresRef,p);
  else {
   players.push({...p,id:jogadorId});
   localStorage.setItem('coruja_players',JSON.stringify(players));
   renderAll();
  }
  ...
  console.error(e);
  alert('Não foi possível cadastrar o jogador. Verifique a conexão e tente novamente.');
 }
}
...
function renderAll(){
 let s=stats();
 $('cConfirmou').textContent=s.confirmou;
 $('cPresente').textContent=s.presente;
 $('cSem').textContent=s.sem;
 $('cNao').textContent=s.nao;
 $('drawCount').textContent=s.presente;
 $('inviteText').value=inviteMessage();
 renderClock();
 renderDashboard();
 renderPlayersList();
 renderPlayersTable();
...
function predictedTeamSizes(total){
 return getTeamSizes(total);
}
function dashboardSummary(total){
 if(total<=0) return 'Aguardando a chegada dos jogadores para calcular a rodada.';
 const sizes=predictedTeamSizes(total);
 const teamsCount=sizes.length;
 const incomplete=sizes.find(size=>size<6);
 let text=`Hoje temos ${total} jogador${total===1?'':'es'} presente${total===1?'':'s'}. `;
 text+=`Ser${teamsCount===1?'á':'ão'} formado${teamsCount===1?'':'s'} ${teamsCount} time${teamsCount===1?'':'s'}. `;
 if(incomplete) text+=`O último time começará com ${incomplete} jogador${incomplete===1?'':'es'}. `;
 else text+='Todos os times ficarão completos com 6 jogadores. ';
 if(teamsCount>=2) text+='Azul e Vermelho começam jogando.';
 return text;
}
function renderDashboard(){
 const total=players.filter(p=>p.presente && (p.status||'ativo')==='ativo').length;
 const confirmou=players.filter(p=>p.whatsapp==='confirmou').length;
 const sizes=predictedTeamSizes(total);
 const times=sizes.length;
 const mensalistas=players.filter(p=>(p.tipo||'').toLowerCase().includes('mensal')).length;
 const avulsosPresentes=players.filter(p=>(p.tipo||'').toLowerCase().includes('avul')&&p.presente).length;
 const receita=mensalistas*65+avulsosPresentes*20;
 const atraso=players.filter(p=>p.presente&&p.atraso).length;
 const pct=teams.length?balancePct(teams):null;

 const map={
  dashDate:new Intl.DateTimeFormat('pt-BR',{weekday:'long',day:'2-digit',month:'long',year:'numeric'}).format(new Date()),
  dashStatus:firestoreOk?'Firestore online':'Modo local',
  dashPresentes:String(total),
  dashConfirmados:String(confirmou),
  dashTimes:String(times),
  dashReceita:`R$ ${receita.toFixed(2).replace('.',',')}`,
  dashAtrasos:String(atraso),
  dashEquilibrio:pct===null?'Aguardando sorteio':`${pct}%`,
  dashSummary:dashboardSummary(total)
 };
 Object.entries(map).forEach(([id,value])=>{const el=$(id); if(el) el.textContent=value});

 const progress=$('dashProgress');
 if(progress){
  const max=Math.max(25,total,1);
  progress.style.width=`${Math.min(100,(total/max)*100)}%`;
 }
 const connection=$('dashConnection');
 if(connection){
  connection.classList.toggle('online',firestoreOk);
  connection.classList.toggle('offline',!firestoreOk);
 }
 const drawState=$('dashDrawState');
 if(drawState) drawState.textContent=teams.length?'✅ Sorteio realizado':'⏳ Aguardando sorteio';
}
...
function bindTableEdit(){
 document.querySelectorAll('.saveEdit').forEach(b=>b.onclick=async()=>{
  ...
  const draft={
   ...p,
   nome:document.querySelector(`.editName[data-id="${id}"]`).value,
   tipo:document.querySelector(`.editType[data-id="${id}"]`).value,
   nivel:Number(document.querySelector(`.editLevel[data-id="${id}"]`).value||5)
  };
  const validation=validatePlayerData(draft,id);
  if(!validation.ok){alert(validation.message); return;}

  createPlayersBackup(`antes de editar ${p.nome}`);
  Object.assign(p,validation.data);
  try{
   await savePlayer(p);
 ...
 document.querySelectorAll('.deletePlayer').forEach(b=>b.onclick=()=>removePlayer(b.dataset.id));
}
function drawTeams(){
